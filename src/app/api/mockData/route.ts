...
const generateJob = (status: 'Queued' | 'Running' | 'Completed' | 'Failed', backendName?: string): any => {
    ...
    const job: any = {
        id: jobId,
        backend: backendName || backendNames[getRandomInt(0, backendNames.length - 1)],
        status: status,
...
const statsData = () => {
    ...
    const backendDetails = backendNames.map(backendName => {
        ...
        return {
            name: backendName,
            jobs: [
                ...Array.from({ length: queuedCount }, () => generateJob('Queued', backendName)),
                ...Array.from({ length: runningCount }, () => generateJob('Running', backendName)),
                ...Array.from({ length: completedCount }, () => generateJob('Completed', backendName)),
                ...Array.from({ length: failedCount }, () => generateJob('Failed', backendName)),
            ],
            predictions: {
                predicted_load,
